...
// Builds the per-leg brief content: the decision line, the figure rows and the
// plain-English analysis. Module-level so the on-screen brief and the printable
// PDF view render from exactly the same source.
function legExplanation(leg,r,i,sym,reserveFuel){
  const depP=toNum(r.depP||0),arrP=toNum(r.arrP||0);
  const diff=toNum(r.priceDiff||0);
  const be=toNum(r.breakEven||0);
  const hrs=toNum(r.hrs||0);
  ...
  if(depP>0&&arrP>0) mathRows.push({l:"Price difference",v:(diff<0?"−":"+")+sym+Math.abs(diff).toFixed(3)+"/lb  "+(diff>0?"cheaper at "+leg.from:"cheaper at "+leg.to)});
  ...
  if(r.tankerLbs>0){
    ...
    if(depP>0) mathRows.push({l:"Cost to purchase extra fuel",v:fM(r.tankerLbs*depP,sym)+" at "+leg.from});
    if(arrP>0) mathRows.push({l:"Cost if bought at destination",v:fM(r.tankerLbs*arrP,sym)+" at "+leg.to});
  }
  ...
  if(r.savings!==0) mathRows.push({l:"Net savings / cost",v:(r.savings<0?"−":"+")+fM(r.savings,sym),highlight:true,color:r.savings>0?C.green:C.red});
  ...
  if(r.decision==="MUST TANKER"){
    ...
    narrative.push("Net result: loading "+fL(r.tankerLbs)+" at "+leg.from+" saves "+fM(r.savings,sym)+" compared to buying at "+leg.to+".");
  } else {
    // NO TANKER
    if(r.fobCoversTrip){
...
// ── Printable brief → PDF ─────────────────────────────────────────────────
// window.print() against an @media print stylesheet rather than a PDF library:
// it is the one path that reliably reaches "Save as PDF" from the iOS Safari
// share sheet, and it needs no extra CDN dependency. The printable markup is
// appended to <body> (outside #root) and the stylesheet hides #root for print,
// so none of the app chrome — nav tabs, buttons, inputs — reaches the page.
const PRINT_HOST_ID="e6b-print";
const PRINT_STYLE_ID="e6b-print-style";
const PRINT_CSS=`
...
#${PRINT_HOST_ID}{color:#000;background:#fff;font:11pt/1.45 -apple-system,BlinkMacSystemFont,"Segoe UI",Helvetica,Arial,sans-serif;
  -webkit-print-color-adjust:exact;print-color-adjust:exact}
#${PRINT_HOST_ID} h1,#${PRINT_HOST_ID} h2,#${PRINT_HOST_ID} h3,#${PRINT_HOST_ID} p,#${PRINT_HOST_ID} dl,#${PRINT_HOST_ID} dd,#${PRINT_HOST_ID} table{margin:0}
.pb-hd{border-bottom:2.5px solid #000;padding-bottom:8px;margin-bottom:12px}
.pb-hd h1{font-size:17pt;letter-spacing:1.2px;font-weight:800}
.pb-route{font-size:13pt;font-weight:700;margin-top:5px;letter-spacing:.4px}
.pb-meta{font-size:9pt;color:#444;margin-top:3px}
.pb-verdict{display:inline-block;margin-top:8px;padding:4px 12px;border:2px solid #000;
  font-size:12pt;font-weight:800;letter-spacing:1.4px}
.pb-verdict.neg{border-style:double;border-width:4px}
.pb-sec{margin-bottom:13px;break-inside:avoid;page-break-inside:avoid}
.pb-sec h2,.pb-leg h2{font-size:10pt;letter-spacing:1.3px;text-transform:uppercase;border-bottom:1px solid #000;
  padding-bottom:3px;margin-bottom:6px;font-weight:800}
.pb-sum{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:5px}
.pb-sum div{border:1px solid #999;padding:5px 8px}
.pb-sum dt{font-size:7.5pt;letter-spacing:.7px;text-transform:uppercase;color:#444}
.pb-sum dd{font-size:11.5pt;font-weight:700;margin:1px 0 0;font-variant-numeric:tabular-nums}
.pb-note{font-size:8.5pt;color:#333;font-style:italic;margin-top:4px!important}
.pb-leg{margin-bottom:14px;break-inside:avoid;page-break-inside:avoid}
.pb-leg h2{display:flex;justify-content:space-between;align-items:baseline;gap:12px}
.pb-leg-sub{font-size:8.5pt;color:#444;font-weight:400;letter-spacing:.2px;text-transform:none}
.pb-dec{border:1.5px solid #000;background:#f2f2f2;padding:5px 9px;font-size:11pt;font-weight:800;margin-bottom:6px}
.pb-fig{width:100%;border-collapse:collapse;font-size:8.8pt;font-variant-numeric:tabular-nums}
.pb-fig td{padding:1.6px 0;vertical-align:top;border-bottom:1px dotted #ccc}
.pb-fig td.k{color:#333;padding-right:10px}
.pb-fig td.v{text-align:right;white-space:nowrap;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
.pb-fig tr.hi td{font-weight:800;border-bottom:1px solid #000}
.pb-wt{border:1.5px solid #000;padding:5px 9px;margin-top:6px;font-size:9pt;font-variant-numeric:tabular-nums}
.pb-wt b{letter-spacing:.4px}
.pb-cap{margin-top:3px!important;font-size:8.8pt;font-weight:700}
.pb-an{margin-top:7px}
.pb-an h3{font-size:8pt;letter-spacing:1.1px;text-transform:uppercase;color:#333;margin-bottom:2px;font-weight:800}
.pb-an p{font-size:9pt;margin-bottom:3px!important;text-align:justify}
.pb-ft{border-top:1.5px solid #000;padding-top:6px;margin-top:4px;font-size:8.5pt;color:#333;
...
// "KABE-KIAD-LGMK-LFMN_tanker-brief_2026-08-19" — used as the document title so
// the print dialog and the iOS share sheet pre-fill a meaningful file name.
function briefFileStem(legs){
  const codes=[];
  if(legs[0]&&legs[0].from)codes.push(legs[0].from);
  for(const l of legs)if(l.to)codes.push(l.to);
  const route=codes.map(c=>String(c).toUpperCase().replace(/[^A-Z0-9]/g,"")).filter(Boolean).join("-")||"TRIP";
  const d=new Date(),p2=n=>String(n).padStart(2,"0");
  return`${route}_tanker-brief_${d.getFullYear()}-${p2(d.getMonth()+1)}-${p2(d.getDate())}`;
}
...
function buildPrintHtml(brief){
  ...
  const zfwUsed=first.zfw||toNum(zfw,0);
  const sum=[
    ["Net Savings",(pos?"+":"−")+fM(totalSavings,sym)],
  ...
  const legHtml=legs.map((leg,i)=>{
    ...
    const{action,mathRows,narrative}=legExplanation(leg,r,i,sym,reserveFuel);
    const sub=[leg.distNm?toNum(leg.distNm).toLocaleString()+" nm":"",
               leg.cruiseAltFt?"FL"+Math.round(toNum(leg.cruiseAltFt)/100):"",
               leg.plannedBurnLbs?"planned burn "+fL(leg.plannedBurnLbs):"planned burn "+fL(r.baseBurn)+" (computed)"]
              .filter(Boolean).join(" · ");
    const rows=mathRows.map(row=>
      `<tr class="${row.highlight?"hi":""}"><td class="k">${esc(row.l)}</td><td class="v">${esc(row.v)}</td></tr>`).join("");
    const mtowOk=!r.mtow||r.takeoffWt<=r.mtow+0.5;
    const mlwOk=!r.mlw||r.landingWt<=r.mlw+0.5;
    const wt=`<b>Weight check</b> — Takeoff ${esc(fL(r.takeoffWt))} / MTOW ${esc(fL(r.mtow))} ${mtowOk?"✓":"✗ OVER"}`
      +(r.mlw>0?` &nbsp;·&nbsp; Landing ${esc(fL(r.landingWt))} / MLW ${esc(fL(r.mlw))} ${mlwOk?"✓":"✗ OVER"}`:"")
      +(r.capNote?`<p class="pb-cap">▲ ${esc(r.capNote)}</p>`:"");
    return`<section class="pb-leg">
      <h2><span>Leg ${i+1} — ${esc(leg.from||"—")} → ${esc(leg.to||"—")}</span><span class="pb-leg-sub">${esc(sub)}</span></h2>
      <div class="pb-dec">${esc(action)}</div>
      <table class="pb-fig"><tbody>${rows}</tbody></table>
      <div class="pb-wt">${wt}</div>
      ${narrative.length?`<div class="pb-an"><h3>Analysis</h3>${narrative.map(n=>`<p>${esc(n)}</p>`).join("")}</div>`:""}
    </section>`;
  }).join("");

  return`<div class="pb-hd">
      <h1>E6B TANKERING BRIEF</h1>
      <div class="pb-route">${esc(route)}</div>
      <div class="pb-meta">${esc(aircraft.name||"GV")} · Generated ${esc(ts)}</div>
      <div class="pb-verdict${pos||mustTanker?"":" neg"}">VERDICT: ${esc(verdict)}</div>
    </div>
    <section class="pb-sec">
      <h2>Trip Summary</h2>
      <dl class="pb-sum">${sum.map(([k,v])=>`<div><dt>${esc(k)}</dt><dd>${esc(v)}</dd></div>`).join("")}</dl>
      ${first.zfwAssumed?`<p class="pb-note">No ZFW or payload entered — weights assume basic operating weight ${esc(fL(zfwUsed))} with nothing on board, so the Max Landing Weight check is optimistic.</p>`:""}
    </section>
    ${legHtml}
    <div class="pb-ft"><span>For planning purposes only · Verify with dispatch</span><span>${esc(ts)}</span></div>`;
}
...
function printBrief(brief){
  ...
  try{
    const doc=document;
    // Replace any host from a previous print rather than stacking them up.
    const old=doc.getElementById(PRINT_HOST_ID);if(old)old.remove();
    if(!doc.getElementById(PRINT_STYLE_ID)){
      const st=doc.createElement("style");st.id=PRINT_STYLE_ID;st.textContent=PRINT_CSS;
      doc.head.appendChild(st);
    }
    const host=doc.createElement("div");
    host.id=PRINT_HOST_ID;
    host.innerHTML=buildPrintHtml(brief);
    doc.body.appendChild(host);
    const prevTitle=doc.title;
    doc.title=briefFileStem(brief.legs);
    let restored=false;
    const restore=()=>{if(restored)return;restored=true;doc.title=prevTitle;};
    // afterprint is unreliable on iOS, so also watch the print media query and
    // keep a long backstop. The host is display:none on screen, so a late
    // cleanup is harmless either way.
    try{window.addEventListener("afterprint",restore,{once:true});}catch(e){}
    try{
      const mql=window.matchMedia("print");
      const onChange=e=>{if(!e.matches){restore();mql.removeEventListener&&mql.removeEventListener("change",onChange);}};
    ...
    <div style={{position:"fixed",inset:0,zIndex:2000,background:C.bg,overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
      ...
      <div style={{background:C.panel,padding:"14px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:1,borderBottom:"1px solid "+C.border}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          <button onClick={()=>printBrief(brief)}
            style={{padding:"4px 10px",borderRadius:6,fontSize:11,fontWeight:700,
              background:C.panel,color:C.sub,border:"1px solid "+C.border,cursor:"pointer"}}>
            ⤓ PDF
          </button>
      ...
      <div style={{padding:"14px 14px 60px"}}>
        ...
        {legs.map((leg,i)=>{
          ...
          const{action,icon,bg,lc,mathRows,narrative}=legExplanation(leg,r,i,sym,reserveFuel);
          return(
...
export default function E6B(){
  ...
  // The PDF always reflects the brief as currently calculated.
      ...
      <div style={{padding:wide?"20px 24px 100px":"14px 14px 100px"}}>
        {screen==="calc"&&<>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:14}}>
            ...
            <button onClick={exportBriefPdf} disabled={!canExportPdf}
              title={canExportPdf?"Print or save the brief as a PDF":"Run a calculation first"}
              style={{background:"transparent",border:"1px solid "+C.border,borderRadius:8,padding:"8px 14px",
                color:canExportPdf?C.muted:C.muted+"66",fontSize:12,fontWeight:600,
